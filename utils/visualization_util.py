...
def visualize_predictions(video_path, predictions, save_path):
    ...
    anim = FuncAnimation(fig, update, frames=np.arange(0, len(frames), 10), interval=1, repeat=True)
    
    if save_path:
        plt.show()
        # 저장. 
        #writer = anim.FFMpegWriter['ffmpeg'](fps=25)
        anim.save(save_path, writer = 'ffmpeg', fps=30, dpi=128)
